...
export default class Scan_FindStock extends React.Component {
      ...
      [
        {
          ...
          onPress: async () => {
            try {
              await AsyncStorage.setItem('KeyLastScanerFind', JSON.stringify({ lastScannedUrl: this.state.lastScannedUrl }))
              navigate('FindStock')
              this.ResetState();
            } catch (error) {
              console.log(error)
