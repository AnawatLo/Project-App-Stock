...
export default class Scan_StockOut extends React.Component {
          ...
          [
            {
              ...
              onPress: async ()=> {
                try {
                  await AsyncStorage.setItem('KeyLastScanerStockOut', JSON.stringify({ lastScannedUrl: this.state.lastScannedUrl }))
                  navigate('StockOut')
                  this.ResetState();
                } catch (error) {
                  console.log(error)
